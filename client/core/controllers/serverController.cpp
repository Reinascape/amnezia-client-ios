    ...
    case DockerContainer::MtProxy: {
        ...
        QString script =
                QString(
                        ...
                        "CLIENTS=$(sudo docker exec amnezia-mtproxy sh -c 'curl -s --max-time 3 "
                        "http://localhost:2398/stats 2>/dev/null | grep -o \"total_special_connections:[0-9]*\" | cut "
                        "-d: -f2' 2>/dev/null); "
                        // 4. Last config refresh — modification time of proxy-multi.conf
                        "CONF_TIME=$(sudo docker exec amnezia-mtproxy sh -c 'stat -c \"%y\" /data/proxy-multi.conf "
